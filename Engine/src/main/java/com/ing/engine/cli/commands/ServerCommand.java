    ...
    static class MCPServer {
        ...
        private String handleResourcesRead(String id, String params) {
            ...
            if ("ingenious://actions".equals(uri)) {
                try {
                    ...
                    String response =
                        "{\"contents\":[{\"uri\":\"ingenious://actions\",\"mimeType\":\"application/json\",\"text\":" +
                        escapeJsonString(actions) +
                        "}]}";
                    return createSuccessResponse(id, response);
                } catch (Exception e) {
        ...
        private String projectInfo(Map<String, String> args) {
            ...
            return String.format(
                "{\"name\":\"%s\",\"path\":\"%s\",\"scenarios\":%d,\"testCases\":%d}",
                projectDir.getName(),
        ...
        private String showTestCase(Map<String, String> args) {
            ...
            File tcFile = new File(
                new File(new File(project, "TestPlan"), parts[0]),
                parts[1] + ".csv"
            );
            ...
            // Read and parse CSV
            StringBuilder sb = new StringBuilder(
                "{\"scenario\":\"" + parts[0] + "\",\"testCase\":\"" + parts[1] + "\",\"steps\":["
            );
        ...
        private String listActions(Map<String, String> args) {
            ...
            for (int i = 0; i < actions.length; i++) {
                if (
                    category != null &&
                    !actions[i][1].toLowerCase().contains(category.toLowerCase())
                ) continue;
                if (i > 0) sb.append(",");
        ...
        private String searchActions(Map<String, String> args) {
            ...
            for (String[] action : actions) {
                if (
                    action[0].toLowerCase().contains(queryLower) ||
        ...
        private String actionInfo(Map<String, String> args) {
            ...
            return String.format(
                "{\"name\":\"%s\",\"objectType\":\"WebElement\",\"description\":\"Performs %s action\",\"inputRequired\":true}",
                action,
                action
            );
        }
        ...
        private String latestReport(Map<String, String> args) {
            ...
            return String.format(
                "{\"runId\":\"%s\",\"date\":\"%s\"}",
                latest.getName(),
                new java.util.Date(latest.lastModified())
            );
        }
        ...
        private String getToolInputSchema(String toolName) {
            ...
            if (
                toolName.contains("project") ||
                toolName.contains("scenario") ||
                toolName.contains("testcase") ||
                toolName.contains("report")
            ) {
        ...
        private int countTestCases(File dir) {
            ...
            for (File f : files) {
                if (f.isFile() && f.getName().endsWith(".csv")) count++; else if (
                    f.isDirectory()
        ...
        private String extractJsonValue(String json, String key) {
            ...
            while (
                valueStart < json.length() && Character.isWhitespace(json.charAt(valueStart))
            ) valueStart++;
        ...
        private String extractJsonObject(String json, String key) {
            ...
            for (int i = braceStart; i < json.length(); i++) {
                if (json.charAt(i) == '{') depth++; else if (json.charAt(i) == '}') depth--;
                if (depth == 0) {
                    braceEnd = i;
                    break;
                }
